...
import { minifyJS } from '../../../utils/developer/formatters';
...
export default function JSMinifier() {
  ...
  return (
    <ToolLayout title={t('tools.jsminifier.name')} description={t('tools.jsminifier.description')}>
      <div className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">{t('common.input')}</label>
          <textarea value={input} onChange={(e) => setInput(e.target.value)} className="w-full h-48 px-4 py-2 border border-gray-300 rounded-lg font-mono text-sm" placeholder="function hello() { console.log('Hello'); }" />
        </div>
